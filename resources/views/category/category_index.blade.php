                ...
                @forelse($posts as $post)
                    <tr class="bg-white border-2 border-gray-200">
                        ...
                        <td class="px-10 py-2 text-center">
                            <form onsubmit="return confirm('Apakah Anda Yakin ?');" action="{{ route('category.destroy', $post->id) }}" method="POST">
                                <button class="bg-blue-500 text-white px-4 py-2 rounded hover:border-indigo-700 text-xs focus:outline-none mr-2"><a href="{{ route('category.edit', $post->id) }}">EDIT</a></button>
                                    @csrf
                                    @method('DELETE')
